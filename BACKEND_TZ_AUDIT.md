...
1. Привести processing timeout базы знаний к требованию 30 минут с переводом
   зависшего документа в `failed` и причиной `Processing timeout`.
2. Добавить отдельный WebSocket route `/ws/notifications` с тем же безопасным
   JWT-handshake и пользовательской группой уведомлений.
3. Удалить или изолировать неиспользуемые старые Django-заготовки `projects`,
