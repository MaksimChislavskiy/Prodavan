...
интеграций доступны как одноразовые команды для ручной диагностики:
...
В production Compose эти задачи запускает один `maintenance_worker` через
команду `run_maintenance_worker --watch`: уведомления проверяются раз в минуту,
Telegram-интеграции и неактивные AI-сессии — раз в пять минут, auth-записи —
раз в сутки. Без `--watch` команда выполняет один полный диагностический проход.
Очистка удаляет истёкшие/отозванные временные auth-записи, завершённые записи
почтовой очереди, освободившиеся e-mail-резервации и auth-аудит старше 366 дней.
AI-chat сессии закрываются после периода неактивности из
`AI_CHAT_SESSION_IDLE_MINUTES` (по умолчанию 30 минут), но история не удаляется:
