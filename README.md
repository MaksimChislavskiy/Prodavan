...
Backend CRM-системы для работы с клиентами и продажами. Проект объединяет рабочие пространства, контакты, сделки, задачи, сообщения, уведомления, Telegram-интеграцию и AI-автоматизацию.

## Что реализовано

- REST API на Django REST Framework;
- авторизация и JWT, регистрация и восстановление доступа;
- рабочие пространства и разграничение данных между ними;
- контакты, сделки и задачи;
- сообщения и уведомления в реальном времени через Django Channels / WebSocket;
- Telegram-интеграция и очереди входящих/исходящих сообщений;
- AI-автоматизация и база знаний;
- фоновые workers с повторными попытками и контролем ошибок;
- PostgreSQL для production, Redis для Channels/cache/rate limits;
- S3-compatible хранилище для файлов;
- health checks, correlation ID и production-ориентированное логирование;
- отдельные настройки безопасности для production-окружения.

## Стек

`Python` · `Django` · `Django REST Framework` · `Django Channels` · `PostgreSQL` · `Redis` · `WebSocket` · `JWT` · `S3` · `Docker`

Ниже находится техническая документация по отдельным подсистемам backend.
