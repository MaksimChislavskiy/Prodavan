...
def _validate_content(uploaded_file, extension):
    if extension == '.pdf':
        ...
        return header == b'%PDF-'
    if extension == '.docx':
        return _validate_docx_container(uploaded_file)
...
def prepare_upload(uploaded_file):
    ...
    if not ALLOWED_FILE_NAME.fullmatch(name):
        raise KnowledgeServiceError(
            'INVALID_FILE_NAME',
    ...
    if extension not in SUPPORTED_FORMATS:
        raise KnowledgeServiceError(
            ...
            'Поддерживаются только PDF, TXT, DOCX, CSV.',
        )

    sha256, size = _hash_and_measure(uploaded_file)
    if size == 0:
        raise KnowledgeServiceError('EMPTY_FILE', 'Файл не должен быть пустым.')
    if not _validate_content(uploaded_file, extension):
        raise KnowledgeServiceError(
            'INVALID_FILE_CONTENT',
            'Формат файла не соответствует его содержимому или файл повреждён.',
        )
...
def storage_usage(workspace):
    aggregate = active_documents(workspace).aggregate(
        files_count=Count('id'),
        used_bytes=Sum('size_bytes'),
    )
    return {
        'files_count': aggregate['files_count'] or 0,
        'files_limit': MAX_FILES_PER_WORKSPACE,
        'used_bytes': aggregate['used_bytes'] or 0,
        'limit_bytes': MAX_WORKSPACE_STORAGE,
    }
...
def broadcast_document_status(document, event='knowledge_document_status'):
    ...
    transaction.on_commit(
        lambda: broadcast_workspace_event(document.workspace_id, payload),
    )
...
def create_knowledge_documents(
    *,
    ...
    if len(uploaded_files) > MAX_FILES_PER_UPLOAD:
        raise KnowledgeServiceError(
            'TOO_MANY_FILES',
            'Можно загружать не более 20 файлов одновременно.',
        )
    ...
    try:
        with transaction.atomic():
            ...
            if usage['files_count'] + len(prepared) > MAX_FILES_PER_WORKSPACE:
                raise KnowledgeServiceError(
                    'FILE_COUNT_LIMIT',
                    'Достигнут лимит количества файлов (1000). Удалите ненужные.',
                )
            if usage['used_bytes'] + batch_size > MAX_WORKSPACE_STORAGE:
                raise KnowledgeServiceError(
                    'STORAGE_LIMIT',
            ...
            for item in prepared:
                document = KnowledgeDocument(
                    ...
                    uploaded_by=user,
                    uploaded_by_identifier=user.id,
                ...
                saved_names.append(document.file.name)
                document.save()
            ...
            for document in documents:
                broadcast_document_status(
                    document,
                    event='knowledge_document_created',
                )
    except Exception:
    ...
    *,
    ...
    with transaction.atomic():
        ...
        if document.status != KnowledgeDocumentStatus.FAILED:
            raise KnowledgeServiceError(
                'DOCUMENT_NOT_FAILED',
                'Повторная обработка доступна только для документов с ошибкой.',
                status_code=409,
            )
        ...
        AIAuditLog.objects.create(
            ...
            user=user,
            user_identifier=user.id,
            action=AIAuditAction.DOCUMENT_RETRY,
            changes={
                'document_id': str(document.id),
