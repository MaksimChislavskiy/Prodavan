...
class RealtimeChatTests(TransactionTestCase):
    ...
    def test_chat_socket_does_not_receive_personal_notification_event(self):
        async_to_sync(self._no_personal_notification_scenario)()

    async def _no_personal_notification_scenario(self):
        communicator = WebsocketCommunicator(
            application,
            f'/ws/chat?token={self.access_token}',
            headers=self._headers(),
